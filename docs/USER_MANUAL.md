...
2. **Template Processing**: Inject sampled parameters into text-based input decks via delimited token replacement (`FlagsMap`) or dynamic Jinja2 templates.
...
scaled_density = density * 1000.0   # [18500.0, 19000.0, 19500.0] kg/m3
offset = density + 0.1

# Rich comparisons yield boolean lists
high_density = density > 19.0        # [False, False, True]
```
...
# Export and import tabular data (.csv, .xlsx, .parquet, .json)
db.export_to_file("core_sampling.parquet")
db.export_to_file("core_sampling.csv")

# Import data into a fresh Database
db_new = Database.import_from_file("core_sampling.csv")
```
...
# Generates all 6 combinations as an aligned Database
grid_db = sweeper.construct()
print(f"Generated {len(grid_db)} parameter rows.")
```

### Advanced Sampling: Latin Hypercube & Halton Sequences
Generate space-filling low-discrepancy sequences for uncertainty quantification:
...
lhs_samples = latin_hypercube_sample(bounds, n=100, seed=42)

# Multi-dimensional Halton sequence
halton_points = halton_sample(n=100, dimensions=3)
```
...
# Template content in mcnp_input.inp:
# c Fuel Material Card
# m1  92235.04c #WF#  92238.04c #(1.0 - WF)#
# c Density Card
# *c Cell Card
# 1  1  -#RHO#  -100
...
### Jinja2 Templating
For dynamic loops, conditionals, and expressions:
```python
from labeeb.utils.file_io import File

template = File("relap_deck.jinja2")
rendered_text = template.render_jinja(
    power=20.0,
...
case.add_file(File("model.template"))

case.exe_cmd = ["python", "simulate.py"]
...
regex_harvester = RegexHarvester(
    target_file="outp",
    patterns={
        "keff": r"final keff estimate\s+=\s+([0-9\.]+)",
        "keff_std": r"standard deviation\s+=\s+([0-9\.]+)",
    },
    types={"keff": float, "keff_std": float}
)

# Extract specific columns from CSV output
csv_harvester = CsvHarvester(
    target_file="results.csv",
    columns={"T_peak": "Max_Clad_Temp", "Pressure": "Outlet_P"}
)

# Custom programmatic harvester function
def parse_custom(cwd: str) -> dict:
    with open(f"{cwd}/summary.txt") as f:
        return {"mdnbr": float(f.readline().split("=")[1])}

callable_harvester = CallableHarvester(fn=parse_custom)
```
...
manifest = CampaignManifest(
    ...
    commands=["sim_executable deck.inp"],
    execution={
...
# Inspect execution status counts
print(campaign.status())  # {'SUCCESS': 2, 'FAILED': 0}
...
def feedback_step(unit, **kwargs):
    ...
    print(f"Relaxed POWER: {relaxed_power}")
...
result = coupler.run_to_convergence(
    max_exec=20,
    check_fn=lambda c: abs(c.database["POWER"][0] - 20.0) < 1e-3,
    error_on_max_exec=True  # Raises CouplingError if not converged in 20 passes
)
```
...
correlations = correlation_analysis(
    inputs={"RHO": [18.0, 19.0, 20.0], "TEMP": [300, 320, 340]},
...
Here is an end-to-end, runnable Python script illustrating a complete uncertainty study using Labeeb:
...
def run_case_study():
    with tempfile.TemporaryDirectory() as workspace:
        ...
        # 2. Generate Latin Hypercube parameter design
        samples = latin_hypercube_sample([(18.5, 19.5), (0.015, 0.025)], n=10, seed=123)
        rhos = [s[0] for s in samples]
        wfs = [s[1] for s in samples]
        ...
        print("Online summary:", memory.online_summary(metrics=["RHO", "WF"]))
