...
* **Failure/output semantics**: a failing case is recorded (FAILED catalog row, `case_failure` event, status registry) and the run continues with remaining rows; `Campaign.run()` returns all results (including FAILED) instead of raising per-case. Required outputs (`output_files` CSV columns, non-optional harvesters) raise `CaseExecutionError`; `optional=True` harvesters record `None` when the file was not produced. Catalog and plot failures are logged and skipped. Failure handling is policy-driven per case (`command_failure_policy`: stop/continue/retry with `max_attempts`; `harvest_failure_policy`: stop/continue; defaults preserve classic fail-fast semantics) and every policy still records the failure in execution history, results, and catalog rows.
